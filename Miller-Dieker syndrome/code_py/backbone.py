...
class Human_Genes_Graph_Analysis:
    ...
    def preprocessing_dataset(self, homo_sap=True, drop_duplicates=True, remove_self_loops=True, write_txt = True):
        ...
        """
        self.homo_sapiens_genes = pd.read_csv(self.data_path+'BIOGRID-ORGANISM-Homo_sapiens-4.4.216.tab3.txt', sep='\t', header=0,low_memory=False)
        #print(self.homo_sapiens_genes)
        if homo_sap:
        ...
        if write_txt:
            self.homo_sapiens_genes[['Official Symbol Interactor A', 'Official Symbol Interactor B']].to_csv(self.folder_path +'data/Biogrid_4.4.216.txt', header=None, index=None, sep=' ', mode='a')
        if drop_duplicates:
            #print(self.homo_sapiens_genes)
            self.homo_sapiens_genes = self.homo_sapiens_genes.drop_duplicates()
            #self.homo_sapiens_genes = self.homo_sapiens_genes[['Official Symbol Interactor A', 'Official Symbol Interactor B']].apply(sorted, axis=1)
            self.homo_sapiens_genes = self.homo_sapiens_genes.drop_duplicates()
            print(len(self.homo_sapiens_genes))
    ...
    # ============================  OPEN PREPROCESSED FILE FROM CYTOSCAPE
    def openfromcytoscape_dataset(self, file):
        self.homo_sapiens_genes = pd.read_csv(self.data_path+file,
                                              sep=' ',
                                              header=None,
    ...
    # ============================  QUERY DISEASE GENES 
    def query_disease_genes(self,file):
        """
        Filter on the diseases curated dataset according to the input disease query
        """
        self.diseases = pd.read_csv(self.data_path+file, sep='\t')
        self.disease_query =  self.diseases[self.diseases["Disease_id"]==self.disease_ID]
        self.disease_list = list(self.disease_query['Gene'])
        print("Found " + str(len(self.disease_list)) + ' disease genes in ' + str(self.disease_query['Disease'].values[0]))
        return self.disease_query,self.disease_list

    # ============================ QUERY DISEASE GENES EXTENDED
    def query_disease_genes_extendend(self, file):
        """
        Filter on the diseases all genes associatiations dataset according to the input disease query
        """
        self.diseases_ex = pd.read_csv(self.data_path+file, sep='\t')
        self.diseases_ex_filtered = self.diseases_ex[self.diseases_ex['Disease_id']==self.disease_ID]
        return self.diseases_ex_filtered,list(self.diseases_ex_filtered['Gene'].values)

    # ============================  LARGE CONNECTED COMPONENTS TO ADJ 
    def LCC_to_adj(self,dataframe, extract_measures=False):
        """
        ...
        print("Number of nodes: {}".format(self.putative_genes_graph.number_of_nodes()))
        print("Number of edges: {}".format(self.putative_genes_graph.number_of_edges()))
        ...
        self.loop_edges = list(nx.selfloop_edges(self.LCC_sub_graph))
        self.LCC_sub_graph.remove_edges_from(self.loop_edges)
        print(nx.info(self.LCC_sub_graph))
        
        if extract_measures == True:
            eig_centrality = nx.eigenvector_centrality(self.LCC_sub_graph)
            btw_centrality = nx.betweenness_centrality(self.LCC_sub_graph)
            cls_centrality = nx.closeness_centrality(self.LCC_sub_graph)
            degree = self.LCC_sub_graph.degree()
            
            return self.LCC_sub_graph, eig_centrality, btw_centrality, cls_centrality, degree
    ...
    def return_metrics(self, method, pgenes_sub_graph, hs_disease_genes, ds_genes_train, ds_genes_test, print_flag=True,save_dataframe=True, extended_val = False, diffusion_time=None):
        """
        ...
        if method == "DIAMOnD":
            ...
            for i in range(5):
                df = pd.read_csv('cytoscape_diffusion_algorithm/'+diffusion_time+'/diff_'+str(i)+'.csv')
